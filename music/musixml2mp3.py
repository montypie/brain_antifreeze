...
def parse_musicxml_to_midi(musicxml_path: Path, temp_midi_path: Path) -> None:
    """Parses a MusicXML file (.xml, .musicxml, .mxl) and outputs a temporary MIDI stream."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert a MusicXML file (.xml, .musicxml, .mxl) directly to an MP3 audio file."
    )
    parser.add_argument(
        "input",
        type=Path,
        help="Path to input MusicXML file (e.g., inputs/piece_name.xml, .musicxml, or .mxl).",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    # Default output path mapping: inputs/piece_name.xml -> outputs/piece_name.mp3
    if args.output:
    ...
    try:
        parse_musicxml_to_midi(input_path, temp_midi_path)
